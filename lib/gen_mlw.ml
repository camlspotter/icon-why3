...
let generate_is_type_wf (type_decl : type_decl) : logic_decl iresult =
  ...
  let name = name_is_type_wf type_decl.td_ident.id_str in
  ...
  let t_params : param list =
    List.map (fun td_param ->
        ( Loc.dummy_position,
          Some (ident (name_is_type_wf ("'" ^ td_param.id_str))),
          false,
           PTarrow (PTtyvar td_param, Mtypes.ty_bool)))
      type_decl.td_params
  in
  return
    {
...
let generate_is_type_wfs (type_decls : type_decl list) : decl list iresult =
  (* One of type names need to be with [@gen_wf] *)
  let generate =
    List.exists (fun td ->
        List.exists (function
            | ATstr atr -> atr.attr_string = "gen_wf"
            | _ -> false) td.td_ident.id_ats
      ) type_decls
  in
  if generate then (
    let* decls = List.map_e generate_is_type_wf type_decls in
    return [Dlogic decls]
  ) else
    return []
...
module Generator (D : Desc) = struct
  ...
  let known_contract (contract : contract) : fundef =
    ...
    let body =
      ...
      in
      let+ ops = E.mk_any @@ Sort.pty_of_sort Mtypes.s_list_operation in
      wrap_assume
        ~assumption:
          (T.of_expr
  ...
  let unknown_func_def =
    ...
    in
    let body =
      E.mk_if (E.mk_any @@ Sort.pty_of_sort Mtypes.s_bool) (E.var_of_binder ctx)
      @@ let+ st =
           E.mk_any
      ...
      M.fold
        (fun _ c flds ->
          ...
          :: {
               ...
               f_pty = Sort.pty_of_sort Mtypes.s_mutez;
               f_mutable = false;
               f_ghost = false;
             }
          :: flds)
